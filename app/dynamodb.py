...
def write_data_class(table, data: dataclass, require_success=True):
    ...
    try:
        ...
        print(
            f"ERROR: DynamoDB could NOT serialize to {table.table_name} item {item_dict}"
        )
        if require_success:
            raise err
        else:
...
def read_data_class(data_class_type: Type[Any], table, key, print_not_found=True):
    try:
        ...
        print(
            f"ERROR: DynamoDB read_data_class ClientError for key {key} cause for table {table_describe}: {err}"
        )
        raise err
...
def read_all_data_class(
    data_class_type: Type[Any], table, partition_key_name: str, partition_key_value: str
):
    ...
    if "Items" not in response or not response["Items"]:
        print(
            f"ERROR: DynamoDB items NOT found in {table} for partition key {partition_key_value}"
        )
        return None

    items = response["Items"]
    data_class_list = [
        dict_to_dataclass(dict_=item_dict, dataclass_type=data_class_type)
...
class DynamoDBManager:
    ...
    def create_table_with_option(
        ...
        key_schema = [
            {"AttributeName": pk_name, "KeyType": "HASH"},  # Partition key
        ]
        attribute_definitions = [
            {"AttributeName": pk_name, "AttributeType": "S"},  # 'S' stands for String
        ]

        if bool(sk_name) and isinstance(sk_name, str):
            print(f"DynamoDB: creating sort key {sk_name}")
            key_schema.append(
                {"AttributeName": sk_name, "KeyType": "RANGE"}  # Sort key
            )
        ...
        if bool(gsi_attributes) and isinstance(gsi_attributes, list):
            ...
            for attr_name in gsi_attributes:
                index_name = generate_index_name(
                    table_name=table_name, attr_name=attr_name
                )
...
def setup_dynamodb_local():
    ...
    try:
        ...
        pid = subprocess.check_output(
            ["lsof", "-i", f":{port}", "|", "awk", "{print $2}", "|", "tail", "-1"]
        )
        pid = pid.decode(
            "utf-8"
        ).strip()  # Convert bytes to string and remove extra whitespace
