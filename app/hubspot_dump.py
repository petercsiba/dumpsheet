...
def _get(props: Dict[str, Any], field_name: FieldNames):
    return props.get(field_name.value)
...
class HubspotDataEntry:
    ...
    def contact_name(self):
        return (
            _get(self.contact_props, FieldNames.FIRSTNAME)
            + " "
            + _get(self.contact_props, FieldNames.LASTNAME)
        )
...
def extract_and_sync_contact_with_follow_up(
    ...
    return HubspotDataEntry(
        contact_props=contact_response.properties,
        call_props=call_response.properties,
        task_props=task_response.properties,
