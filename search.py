...
def get_most_similar(df, person_embedding, n=2):
    ...
    df['similarities'] = df['ada_embedding'].apply(lambda x: cosine_similarity(np.array(x).reshape(1, -1), search_vector))
    most_similar_df = df.sort_values('similarities', ascending=False).head(n+1)  # n+1 because the person is most similar to themselves
    ...
    most_similar_df['similarities'] = most_similar_df['similarities'].apply(lambda x: x[0].item())
...
def create_embeddings(csv_filepath, output_filepath):
    ...
    all_people_entries: List[PersonDataEntry] = [item for sublist in list_of_lists for item in sublist]  # GPT generated no idea how it works
...
and 
* {}: {}
""".format(person1['name'], person2['name'], person1['name'], person1['text'], person2['name'], person2['text'])
...
if __name__ == "__main__":
    ...
    for index, person in people_df.iterrows():
        ...
        most_similar_people = most_similar_people[most_similar_people['similarities'] > 0.8]

        # If the DataFrame is not empty, print it
        if most_similar_people.empty:
            continue

        intro_candidate = most_similar_people[most_similar_people['similarities'] > 0.87].head(1)
        if not intro_candidate.empty:
            candidate = intro_candidate.iloc[0]
            print(f"Found a good intro candidate {candidate['name']}, lets generate the intro message")
